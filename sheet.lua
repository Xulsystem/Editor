...
local theFont = love.graphics.newFont(20)
theFont:setLineHeight(FONT_LINE_HEIGHT)
sheet.tZone:setFont(theFont, 20)
...
function sheet.draw()
	...
	for _, x, y, w, h in sheet.tZone:eachSelection() do
		love.graphics.rectangle("fill", sheet.x+x, sheet.y+y, w, h)
	end
	...
	for _, text, x, y in sheet.tZone:eachVisibleLine() do
		...
		for s in text:gmatch("[%S%(]+") do
			...
			love.graphics.print(s, sheet.x + div, sheet.y+y)
	...
	love.graphics.rectangle("fill", sheet.x+x, sheet.y+y, 1, h)
